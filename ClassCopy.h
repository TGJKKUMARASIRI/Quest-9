...
public:
    ...
    SavingsAccounts *NextSavingsAccounts;
...
public:
    ...
    //function to display users's account numbers both saving and current
    void DisplayAccountNumbers()
    {
        SavingsAccounts *CurrentSavingsAccounts;
        CurrentSavingsAccounts = HeadSavingsAccounts;
        while (CurrentSavingsAccounts->NextSavingsAccounts != NULL)
        {
            cout << CurrentSavingsAccounts->AccountNumber << endl;
            CurrentSavingsAccounts = CurrentSavingsAccounts->NextSavingsAccounts;
        }
        CurrentSavingsAccounts = HeadSavingsAccounts;
        while (CurrentSavingsAccounts->NextSavingsAccounts != NULL)
        {
...
public:
    ...
    void GetCustomerInfo()
    {
        ...
        while (CustomerInfo.eof() == false)
        {
            ...
            CustomerInfo.getline(Buffer, 128); 
            string No(Buffer);
            for (int i = 0; i < stoi(No); i++)
            {
                CustomerInfo.getline(Buffer, 128);
                string AccNo(Buffer);
                AccNo += ".txt";
                ifstream AccountInfo(AccNo);
                SavingsAccounts *CurrentSavingsAccounts;
                CurrentCustomer->HeadSavingsAccounts = new SavingsAccounts;
                CurrentSavingsAccounts = CurrentCustomer->HeadSavingsAccounts;
                AccountInfo.getline(Buffer, 128);
                CurrentSavingsAccounts->AccountNumber = Buffer;
                AccountInfo.getline(Buffer, 128);
                //CurrentSavingsAccounts->AnualInterestRate = stod(Buffer);
                //readint the last line of the file to get the amount
                while (AccountInfo.eof() == false)
                {
                    AccountInfo.getline(Buffer, 128);
                }
                string Amount(Buffer);
                //CurrentSavingsAccounts->amount = stod(Amount);
                CurrentSavingsAccounts->CustomerName = CurrentCustomer->name;
                CurrentSavingsAccounts->PhoneNumber = CurrentCustomer->phone;
                CurrentSavingsAccounts->NextSavingsAccounts = new SavingsAccounts;
                CurrentSavingsAccounts = CurrentSavingsAccounts->NextSavingsAccounts;
                CurrentSavingsAccounts->NextSavingsAccounts = NULL;
            }
            CustomerInfo.getline(Buffer, 128); 
            string NoC(Buffer);
            for (int i = 0; i < stoi(NoC); i++)
            {
        ...
        while (CurrentCustomer->NextCustomer != NULL)
        {
            ...
            if (ID == GettingTheID(Id))
            {
                ...
                case '1':
                {
                    ...
                    CurrentCustomer->DisplayAccountNumbers();
                    break;
                }
                case '2':
                {
